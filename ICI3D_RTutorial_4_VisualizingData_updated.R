...
library(tidyverse)
botswana.data <- read_csv('datasets/HIV_Botswana.csv')
...
ggplot(botswana.long[botswana.long$year == 2001,], aes(x="", y=prevalence, fill=type))+
	geom_bar(stat = "identity", width = 1) + 
	coord_polar("y", start = 0) +
	theme_void()+ # remove background, grid, numeric labels
...
measles.London <- read_csv("datasets/MeaslesCleanLon.csv")
...
## options.
ggplot(measles.London, aes(x=date, y=cases))+
	geom_point() + geom_line() + ## FIXME TRY geom_line(), geom_point(), geom_line()+geom_point()
	theme_bw()+
...
ggplot(measles.London, aes(x=date, y=cases))+
	...
	annotate("segment",x=vaccine.year,
					 xend=vaccine.year, y=3000, # TRY: change the value of y
					 yend=2500, color="blue",
...
ggplot(measles.London, aes(x=date, y=cases))+
	...
	annotate( "text",  x=vaccine.year, y=5500, # x and y coordinates of the text
...
measles.Liverpool <- read.csv("datasets/MeaslesCleanLP.csv")
...
ggplot(measles.data, aes(x=date, y=cases, group=country, colour=country))+ # while plotting, group the data for each country
	...
	labs(x="Time", y="Weekly measles cases", title="Weekly measles cases in two UK cities")# add a x, y axis label and a title for the graph
...
measles.London<- measles.London%>%
	mutate(month =format(date, format = "%m"))    # TRY "%B", "%m", "%b" and# "%b-%Y".  
#Pick the value
...
ggplot(measles.London, aes(x=month, y=cases, ))+geom_point(shape=1)+ # TRY 1, 4, 5,8, 20
	theme_bw()+
	labs(x="Month", y="Number of cases", # WHAT IS AN APPROPRIATE LABEL?
...
measles.Liverpool <- measles.Liverpool %>%
	mutate(month =format(date, format = "%m"))    # TRY "%B", "%m", "%b" and# "%b-%Y".  

measles.Liverpool.mean <- measles.Liverpool %>%
	group_by(month) %>%
	summarise(mean_cases = mean(cases))

ggplot() +
	geom_line(data=measles.London.mean, aes(x=month, y=mean_cases, group=1), colour="blue") +
	geom_line(data=measles.Liverpool.mean, aes(x=month, y=mean_cases, group=2), colour="red") 

measles.Liverpool.mean$city <- "Liverpool"
measles.London.mean$city <- "London"

combined <- rbind(measles.Liverpool.mean, measles.London.mean)

ggplot() +
	geom_line(data=combined, aes(x=month, y=mean_cases, group=city, colour=city))
...
hookworm <- read_csv("datasets/Hookworms.csv")
...
my.breaks <- pretty(range(hookworm$epg),10000)
...
p1 <- ggplot(hookworm, aes(epg))+stat_bin( binwidth=10)+
	theme_bw()+xlim(0, 10000)+ # set the x axis limit
	labs(x="eggs per gram", title="Egg Per Gram Distribution")

p2 <- ggplot(hookworm, aes(epg))+stat_bin( breaks=seq(0,10000, 1000))+
	theme_bw()+xlim(0, 10000)+ # set the x axis limit
	labs(x="eggs per gram", title="Egg Per Gram Distribution")
...
ggplot(hookworm, aes(epg))+stat_bin( binwidth=100)+
	facet_grid(~shoes) +
	theme_bw()+xlim(0, 10000)+ # set the x axis limit
	labs(x="eggs per gram", title="Egg Per Gram Distribution")
