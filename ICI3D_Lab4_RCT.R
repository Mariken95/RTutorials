...
gen.data(n.subjects = 250, control.mean = 1, treatment.effect = 0.2, outcome.sd = 0.2) 
...
## treatment effect is the same in both groups. outcome score 3x higher.
## same level of improvement when combining severity groups.
# the more severe people are in the treatment group, the more effective the treatment seems
# as long as sample size is large enough the % with high severity should be the same in both arms.
...
head(mutxt)
summary(lm(scar_3W ~ drug, data=mutxt))

mean(mutxt$scar_baseline[mutxt$drug==0])
mean(mutxt$scar_baseline[mutxt$drug==1]) # scar baseline only a little bit smaller in treatment group

summary(lm(scar_3W ~ scar_baseline, data=mutxt)) # strong association between scar at 3w and scar at baseline
summary(lm(scar_baseline ~ drug, data=mutxt)) # no association between scar at baseline and treatment

summary(lm(scar_3W ~ drug + scar_baseline, data=mutxt)) # estimate 0.09, p=0.63
summary(lm(scar_3W ~ drug, data=mutxt)) # estimate -0.07, p=0.84

# so no evidence of a treatment effect on scar size at 3 weeks. no indicated of confounding from baseline scar size 
# (baseline scar size was similar across both treatment groups due to randomisation)
