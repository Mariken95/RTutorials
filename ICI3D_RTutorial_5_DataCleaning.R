...
dat  <-  read_csv("datasets/tutorial5.csv") # read in the "data"
...
print(
	dat
	%>% distinct(ageMonths) # View distinct values for month ## FIXME
	%>% arrange(ageMonths) # Arrange values in ascending order ## FIXME
)
...
print(
	dat
	%>% filter(ageYears>0) # subset the rows for ages of at least 1 year ## FIXME
	%>% group_by(ageMonths) # group by ageMonths
